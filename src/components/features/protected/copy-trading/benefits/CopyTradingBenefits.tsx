...
export default function CopyTradingBenefits() {
  return (
    <div className="flex flex-col-reverse lg:flex-row justify-center items-center lg:items-start gap-4 p-6 self-stretch rounded-[16px] border border-white/3 bg-dark-radial-gradient">
        ...
        <div className="flex flex-col items-start gap-2.5 self-stretch">
          {ACCOUNT_TYPES[1].items.map((item, index) => (
            <div className="flex items-start gap-2.5 self-stretch" key={index}>
              ...
              <Text16 className="font-[400] flex-[1_0_0]">{item}</Text16>
            </div>
          ))}
        </div>
      </div>
